...
class IntervenantSeeder extends Seeder
{
    ...
    {
        $filePath = database_path('seeders/data/intervenants.xlsx');
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            if (empty($rowData['nom_intervenant'])) {
                $this->command->warn("Row $row skipped: 'nom_intervenant' is missing or empty.");
                continue;
            }
        ...
        DB::table('intervenants')->insert($rows);
        $this->command->info(count($rows) . " intervenants inserted successfully.");
    }
}