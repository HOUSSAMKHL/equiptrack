...
class AtelierSeeder extends Seeder
{
    ...
    {
        $filePath = database_path('seeders/data/ateliers.xlsx');
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            if (empty($rowData['numero_atelier'])) {
                continue;
            }
        ...
        DB::table('ateliers')->insert($rows);
        $this->command->info(count($rows) . " ateliers inserted successfully.");
    }
}