...
class FrequenceSeeder extends Seeder
{
    ...
    {
        $filePath = database_path('seeders/data/frequences.xlsx');
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            if (empty($rowData['type_frequence'])) {
                $this->command->warn("Row $row skipped: 'type_frequence' is missing or empty.");
                continue;
            }
        ...
        DB::table('frequences')->insert($rows);
        $this->command->info(count($rows) . " frequences inserted successfully.");
    }
}